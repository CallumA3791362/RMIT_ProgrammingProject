 ...
 * $u0 : ID, display_name, user_login, c_ip, c_pays, c_ville, i_sex, d_naissance, i_zsex, c_zsex, i_zage_min, i_zage_max, i_zrelation, c_zrelation, i_photo, t_action, sourireIn, contactIn, visite, looking, forwhat, homo, zsex, country, age, agemin, agemax, pause
*/
?>
		...
		<div class="w3-container<?php if(!empty($hideSideMobile)) echo' w3-hide-small'; ?>">
			...
			<?php if($u0->i_photo!=0) { ?>
				...
				<div class="w3-cell w3-cell-middle" style="width:60px;">
					<img class="w3-circle" src="<?php echo $rencDiv['baseurl'].'/portrait/'.floor(($u0->ID)/1000).'/'.Rencontre::f_img(($u0->ID*10).'-mini').'.jpg?r='.rand(); ?>" alt="<?php echo $u0->display_name; ?>" />
				</div>
			<?php } else { ?>
