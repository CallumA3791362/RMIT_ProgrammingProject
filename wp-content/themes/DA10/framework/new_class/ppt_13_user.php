		...
		switch($action){
			...
			case "get_memberships": { 
				...
				if($userdata->ID && !isset($GLOBALS['SHOWALLMEMS'])){		
					...
					if(is_array($cm) && isset($cm[0]) && _ppt($cm[0]['key'].'_repurchase') == "0"){
				...
				while($i < 11){		 	 
					...
					// CHECK FOR DONT SHOW KEY
					if($dontshowkey == $key || $dontshowkey == "mem".$key){
					$i++;
					continue;
					}					 			 
					...
					$list[$i] = array(					
						"key" 		=> $key,
						"name" 		=> trim(stripslashes($name))." -".$dontshowkey,
	 ...
	 case "get_subscribers_followingme": {
		...
		case "bar_reputation": {
			...
			case "get_user_profile_link": {
				...
				if(!is_numeric($order_data)){ return "#"; }
			...
			case "get_address": {		
				...
				// BUILD DISPLAY NAME
				$name = get_user_meta( 'address1', $order_data, true)." ".get_the_author_meta( 'address2', $order_data, true)." ".get_the_author_meta( 'country', $order_data, true);
		...
		case "get_logs": {
			...
			if(!empty($logs)){	
				...
				if(get_post_meta($log->ID,"log_public", true) == 1){ continue; }
					...
					?>
					  <li class="feed-item">
						...
						<span class="desc"><?php echo $d['desc']; ?> - <em class="date"><?php echo $vv['string-small']; ?><?php echo __("ago","premiumpress"); ?></em> </span>
		...
		case "count_offers_pending_post": {
			...
			$d2 = $order_data[1]; // USER ID
				...
				$SQL = "SELECT * FROM ".$wpdb->prefix."posts 
						...
						AND ( mt1.meta_key = 'post_id' AND mt1.meta_value = ('".$d1."') AND mt2.meta_key = 'seller_id' AND mt2.meta_value = ('".$d2."') )						 				
			...
			case "get_account_links": {
				...
				$accountitems = array(
					...
					"offers" => array(
						...
						"name" => __("My","premiumpress")." ".$CORE->LAYOUT("captions","offers"),
					...
					"listings" => array(
						...
						"name" => __("My","premiumpress")." ".$CORE->LAYOUT("captions","2"),
					...
					"messages" => array(
						...
						"name" => __("My Messages","premiumpress"),
					...
					"orders" => array(
						...
						"name" => __("My Invoices","premiumpress"),
...
<script >
    jQuery(document).ready(function() {  jQuery('#field-<?php echo $key; ?>-date1').datetimepicker({format: 'yyyy-mm-dd hh:ii:ss', fontAwesome: 1}); });
        ...
        <input type="input" <?php if(isset($GLOBALS['flag-register'])){ ?>placeholder="<?php echo $label; ?>"<?php } ?> name="custom[<?php echo $key; ?>]" class="form-control rounded-0 <?php echo $eclass; ?>" tabindex="<?php echo $taborder; ?>" value="<?php echo esc_attr( $user_value ); ?>"  id="field-<?php echo $key; ?>" />
		...
		case "textarea": { 
		?>
        <textarea <?php if(isset($GLOBALS['flag-register'])){ ?>placeholder="<?php echo $label; ?>"<?php } ?> name="custom[<?php echo $key; ?>]" rows="10" class="form-control rounded-0 <?php echo $eclass; ?> height:100px" style=" height:100px !important;" tabindex="<?php echo $taborder; ?>" id="field-<?php echo $key; ?>"><?php echo esc_textarea($user_value); ?></textarea>
		<?php
        } break;	
