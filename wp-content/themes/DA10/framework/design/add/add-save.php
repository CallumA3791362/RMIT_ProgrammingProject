...
<hr style="margin: 0px -20px 15px -20px;" />
<a href="javascript:void(0);" class="small float-right" onclick="jQuery('#listing_expiry_date').val('<?php echo date('Y-m-d H:i:s', strtotime(current_time( 'mysql' ) . "+1 minute")); ?>');">Time Now + 1 minute</a>
<h6><?php echo __("Expiry Date","premiumpress") ?></h6>
...
<?php if(isset($_GET['eid']) &&  get_post_meta($_GET['eid'] ,'listing_expiry_date', true) != ""){ ?>
<p><?php echo __("Expires","premiumpress"); ?>: <?php echo do_shortcode('[TIMELEFT postid="'.$_GET['eid'].'" layout="1" text_before="" text_ended="Not Set" key="listing_expiry_date"]'); ?></p>
<?php } ?>
<?php } ?>
<!--------------- INVOICES FOUND --->
