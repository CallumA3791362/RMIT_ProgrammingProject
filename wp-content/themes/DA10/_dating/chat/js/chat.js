...
var Chat =
{
	...
	login: function ()
	{
		...
		jQuery.post(server_path+'ajax/chat.php', {'action': 'login', 'username': Chat._username, time: timestamp}, function(response)
		{
			...
			if (response === '1')
			{
				...
				Chat._show_new_message (false, '', '', '<span class="text_welcome">Welcome</span>, <strong class="text_welcome_username">'+Chat._username+'</strong>!', '', '', 'system');
			}
		});
	},
	...
	_show_new_user: function(photo, username, ulink, uid)
	{
		...
		jQuery('#users-list li:last-child')
			.html( "<div class='row'><div class='col-md-3'><a href='"+ulink+"' target='_blank'>" + photo + "</a></div><div class='col-md-8'><span class='username'>" +  escape_html(username) + '</span>' +
			     
				  (is_admin ? ' <div class="adminoptions"><a href="javascript:void(0);" onclick="Chat.kick(\''+escape_html(username) + '\',\''+uid+'\');" class="admin kick"><i class="fa fa-thumbs-down"></i> Kick for 1 hour</a> <a href="javascript:void(0);" onclick="Chat.ban(\''+escape_html(username) + '\',\''+uid+'\');" class="admin ban"><i class="fa fa-minus-circle"></i> Ban for 1 week</a></div>'  : '<a href="'+ulink+'" target="_blank" class="small">view profile</a>')
	...
	_show_new_message: function(date, photo, username, message, datetext, mycss, system)
	{
		...
		//console.log(date +' -- '+ photo +' -- '+  username +' -- '+  message +' -- '+  datetext +' -- '+  mycss +' -- '+  system);
		...
		if (!system)
		{
			...
			content += '<div class="comment"><p>' + message + '</p></div> <div class="bottombit"><i class="fa fa-clock-o"></i> <span class="chat_date">'+datetext+'</span></div>'; //<span class="chat_username">'+username+'</span> 
	...
	scale_window: function()
	{
		...
		var new_h = window_h - jQuery('#header').height() - jQuery('#send-message').height() - 300;
		...
		jQuery.post(server_path+'ajax/chat.php', {'action': 'check_username', 'username': username, time: timestamp}, function(username_available)
		{
			if (username_available == '0')
			{
				...
				jQuery('#wlt_chatwindow').load(server_path+'chat_window.html?time='+timestamp, function()
				{
					...
					setInterval(function() { Chat.ping(); }, 15500);
					setInterval(function() { Chat.update_users_list(); }, 15000);
				});
