		...
		switch($_POST['search_action']){
			...
			case "search_live":{
				...
				case "search": {				
					...
					// QUERY CHANGES
					if($args['cardtype'] == "admin-user" && function_exists('current_user_can') && current_user_can('administrator') ){
		...
		if($code == $code_answer && $code != ""){
			...
			if(!$userdata->ID){	$userid = 1;}else{	$userid = $userdata->ID; }
				...
				if($_POST['pid'] == ""){
					...
					// SEND
					$CORE->email_send(get_option('admin_email'), __("Contact Form","premiumpress")." - ".date("F j, Y"), $CORE->email_message_filter($Message, array()));
			...
			if(is_array($ppt_coupons) && count($ppt_coupons) > 0 ){
				foreach($ppt_coupons as $key => $field){
					if($_POST['code'] == $field['code']){	
						...
						$amount = intval(strval($_POST['amount'])) - $dc;
