...
<div class="card card-filter">
  <div class="card-body"> 
  <a href="#" data-toggle="collapse" data-target="#collapse_tax-<?php echo strip_tags(str_replace("-","",$taxonomy)); ?>" aria-expanded="true" class="">
   ...
   <select name="tax-make" class="form-control customfilter" id="reg_field_tax_make" name="sort" data-type="select" data-key="taxonomy" onchange="ChangeSearchValues('',this.value,'model__make','tx_model[]','-1','0', 'reg_field_tax_model');  ">
...
?>
      <option value="make-<?php echo $cat->term_id; ?>" <?php if(isset($_GET['tax-make']) && $_GET['tax-make'] == $cat->term_id){ echo "selected=selected"; } ?>>
...
<?php if(isset($_GET['tax-make']) && is_numeric($_GET['tax-make']) ){ ?>
 jQuery(document).ready(function(){ 
   	...
   	ChangeSearchValues('','make-<?php echo esc_attr($_GET['tax-make']); ?>','model__make','tx_model[]','<?php if(isset($_GET['tax-model']) && is_numeric($_GET['tax-model']) ){ echo esc_attr($_GET['tax-model']); }else{ echo "-1"; } ?>','0', 'reg_field_tax_model');
...
<?php } ?>
 ...
 <?php   $count=0;  }elseif(THEME_KEY == "dl" && $taxonomy == "model"){ $count=0; ?>
  ...
  <select name="tax-model" class="form-control customfilter" data-live-search="true" id="reg_field_tax_model"  data-type="select" data-key="taxonomy" onchange="_filter_update();" >
